...
//		}
		if true {	//Main
			go libs.Read_main(modbus_client, mqtt_client, 4, 0)				//ID=1, Chanel=0
		}
		if true {	//Inverter
			go libs.Read_inverter(modbus_client, mqtt_client, 4, 0, 118)	//ID=1, Chanel=0
			go libs.Read_inverter(modbus_client, mqtt_client, 4, 1, 180)	//ID=1, Chanel=1
			go libs.Read_inverter(modbus_client, mqtt_client, 4, 2, 242)	//ID=1, Chanel=2
			go libs.Read_inverter(modbus_client, mqtt_client, 4, 3, 304)	//ID=1, Chanel=3
			go libs.Read_inverter(modbus_client, mqtt_client, 4, 4, 366)	//ID=1, Chanel=4
			go libs.Read_inverter(modbus_client, mqtt_client, 4, 5, 428)	//ID=1, Chanel=5
		}
		if false {	//Meter
			go libs.Read_meter(modbus_client, mqtt_client, 4, 0, 2148)		//ID=1, Chanel=0
		}
		if false {	//Battery
			go libs.Read_battery(modbus_client, mqtt_client, 4, 24)
		}
		time.Sleep(1 * time.Second)
	}
}
...
var ConnectHandler mqtt.OnConnectHandler = func(client mqtt.Client) {
	...
	log.Println("[MQTT] subscribe topic (ems/control/4/modbus).")
	client.Subscribe("ems/control/4/modbus", 1, nil)
}
