...
var modbus_host = "tcp://YOUR_Mobus_Host_Address"
...
//		}
		if true {	//Main
			go libs.Read_main(modbus_client, mqtt_client, 3, 0)				//ID=3, Chanel=0
		}
		if true {	//Inverter
			go libs.Read_inverter(modbus_client, mqtt_client, 3, 0, 118)	//ID=3, Chanel=0
			go libs.Read_inverter(modbus_client, mqtt_client, 3, 1, 180)	//ID=3, Chanel=1
			go libs.Read_inverter(modbus_client, mqtt_client, 3, 2, 242)	//ID=3, Chanel=2
			go libs.Read_inverter(modbus_client, mqtt_client, 3, 3, 304)	//ID=3, Chanel=3
			go libs.Read_inverter(modbus_client, mqtt_client, 3, 4, 366)	//ID=3, Chanel=4
			go libs.Read_inverter(modbus_client, mqtt_client, 3, 5, 428)	//ID=3, Chanel=5
		}
		if true {	//Meter
			go libs.Read_meter(modbus_client, mqtt_client, 3, 0, 2148)		//ID=3, Chanel=0
		}
		if true {	//Battery
			go libs.Read_battery(modbus_client, mqtt_client, 3, 24)
		}
		time.Sleep(1 * time.Second)
	}
}
...
var ConnectHandler mqtt.OnConnectHandler = func(client mqtt.Client) {
	...
	log.Println("[MQTT] subscribe topic (ems/control/3/modbus).")
	client.Subscribe("ems/control/3/modbus", 1, nil)
}
